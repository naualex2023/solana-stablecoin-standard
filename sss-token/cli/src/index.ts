...
async function ensureMint(mintArg?: string): Promise<PublicKey> {
  if (mintArg) {
...
program
  ...
  .action(async (recipientStr: string, amountStr: string, options) => {
    try {
...
program
  ...
  .option("--mint <mint>", "Mint address")
  .action(async (addressStr: string, options) => {
    try {
...
program
  ...
  .action(async (sourceAccountStr: string, options) => {
    try {
  ...
  .action(async (addressStr: string, options) => {
    try {
      ...
      const remaining = formatAmount(
        new BN(minterInfo.quota).sub(new BN(minterInfo.minted)),
        configData.decimals
      );
...
mintersCmd
  ...
  .option("--mint <mint>", "Mint address")
  .action(async (addressStr: string, quotaStr: string, options) => {
    try {
...
program
  ...
  .option("--mint <mint>", "Mint address")
  .action(async (newAuthorityStr: string, options) => {
    try {
...
program
  ...
  .option("--mint <mint>", "Mint address")
  .action(async (addressStr?: string, options?: any) => {
    try {
